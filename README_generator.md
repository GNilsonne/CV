...
- `python3 scripts/render_kid.py` renders a KI-style publication list for the rolling last 5 years as one single list containing only peer-reviewed research articles, writing `output/kid_publist.tex` and, if `pdflatex` is available, `output/kid_publist.pdf`.
